...
const initializeDefaultData = async () => {
    try {
        ...
        console.error('âŒ Error creating default data:', error);
    }
};
...
app.get('/api/resources/:id/download', authenticateToken, async (req, res) => {
    try {
        ...
        const resource = await Resource.findById(req.params.id);
        if (!resource) {
            console.error('âŒ Resource not found:', req.params.id);
            return res.status(404).json({ error: 'Resource not found' });
        }
        ...
        if (!fs.existsSync(resource.filePath)) {
            console.error('âŒ File not found on disk:', resource.filePath);
        ...
        fileStream.on('error', (error) => {
            console.error('âŒ File stream error:', error);
            if (!res.headersSent) {
        ...
        console.error('âŒ Download resource error:', error);
        if (!res.headersSent) {
            res.status(500).json({ error: 'Internal server error' });
        }
    }
